...
{
	if(errorMessage == nil){		
		NSString *humanizedPropertyName = NSLocalizedString([propertyName capitalizedString], @"");
		switch (validationType) {
			case ValidatesExistence:
				errorMessage = [NSString stringWithFormat:@"%@ %@.", humanizedPropertyName,
								NSLocalizedString(@"can't be blank", @"")];
				break;
			case ValidatesAssociated:
				errorMessage = [NSString stringWithFormat:@"%@ %@.", humanizedPropertyName,
								NSLocalizedString(@"is invalid", @"")];
				break;
			...
			case ValidatesNonZeroNumber:
				errorMessage = [NSString stringWithFormat:@"%@ %@.", humanizedPropertyName,
								NSLocalizedString(@"must not be 0", @"")];
				break;				
			case ValidatesNumberInRange:
				errorMessage = [NSString stringWithFormat:@"%@ %@.", humanizedPropertyName,
								NSLocalizedString(@"does not fall within the accepted range", @"")];
				break;
			default:
				errorMessage = [NSString stringWithFormat:@"%@ %@.", humanizedPropertyName, NSLocalizedString(@"is not valid", @"")];
				break;
		}
		[errorMessage retain];
	}
